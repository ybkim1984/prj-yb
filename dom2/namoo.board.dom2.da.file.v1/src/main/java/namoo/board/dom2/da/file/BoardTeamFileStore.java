...
import namoo.board.dom2.da.file.repo.BoardMemberRepository;
import namoo.board.dom2.da.file.repo.BoardTeamRepository;
import namoo.board.dom2.da.file.repo.BoardUserRepository;
...
public class BoardTeamFileStore implements BoardTeamStore {
    ...
    public DCBoardTeam retrieveByName(String name) throws EmptyResultException {
        //
        DCBoardTeam team = teamStore.selectByName(name); 
        if (team == null) {
    ...
    public DCBoardMember retrieveMember(String teamUsid, String memberEmail) throws EmptyResultException {
        ...
        // FIXME: teamUsid, memberEmail로 조회하면서 인덱스 사용을 못함..
        // 인덱스를 사용할 수 있는 방안은? comment 처럼 json으로 변환한 뒤 저장?
    ...
    public List<DCBoardMember> retrieveMembers(String teamUsid) {
        //
        return setMemberTeamName(teamUsid, memberStore.selectByTeamn(teamUsid));
    }
